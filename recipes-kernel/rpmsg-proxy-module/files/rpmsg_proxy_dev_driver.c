...
static struct rpmsg_channel_info proxy_rpmsg_chn = {
	"rpmsg-proxy-chn",
	PROXY_ENDPOINT,
	RPMSG_ADDR_ANY,
};
				...
				loff_t *p_off)
{
	...
	if (filp->f_flags & O_NONBLOCK)
		ret = rpmsg_trysendto(local->ept, kbuf, len, PROXY_ENDPOINT);
	else
		ret = rpmsg_sendto(local->ept, kbuf, len, PROXY_ENDPOINT);
...
static int rpmsg_proxy_dev_rpmsg_drv_probe(struct rpmsg_device *rpdev)
{
	...
	dev_set_name(&local->dev, "rpmsg_proxy%d", ret);
